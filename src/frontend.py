...
if not GEMINI_API_KEY:
    ...
    st.error("🚨 CRITICAL: GEMINI_API_KEY environment variable is not set! Check your .env file.")
    st.stop()
...
def auth_page():
    ...
    with col2:
        st.markdown("<h1 style='color: #E50914; text-align: center; font-weight: 900;'>ScaleRec</h1>", unsafe_allow_html=True)
        st.caption("<div style='text-align: center; color: #b3b3b3; margin-bottom: 20px;'>Sign In to your AI Cinema</div>", unsafe_allow_html=True)
...
# --- 3. UI: MAIN DASHBOARD ---
st.set_page_config(page_title="ScaleRec | Netflix Mode", layout="wide", initial_sidebar_state="expanded")
...
# --- SIDEBAR ---
with st.sidebar:
    st.markdown(f"<h2 style='color: #E50914; font-weight: 900; margin-bottom: 0;'>ScaleRec</h2>", unsafe_allow_html=True)
...
# ==========================================
with tab_feed:
    st.markdown("<h2 style='font-weight: 800; margin-bottom: 0;'>Top Picks for You</h2>", unsafe_allow_html=True)
        ...
        if not selected_movies:
            ...
            with st.spinner("Analyzing neural sequences and ranking candidates..."):
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        for i, item in enumerate(data):
                            with cols[i % 4]:
                                ...
                                genre = raw_genre.replace("|", " • ") if raw_genre != "(no genres listed)" else "Drama"
                    ...
                    st.error("Backend is offline. Ensure your FastAPI server is running (`python src/04_inference.py`).")

# ==========================================
# TAB 2: POPGURU LLM AGENT
# ==========================================
with tab_bot:
    st.markdown("<h2 style='font-weight: 800; margin-bottom: 0;'>PopGuru: AI Entertainment Insider</h2>", unsafe_allow_html=True)
    st.caption("Powered by Gemini 2.5 Flash API — Ask about the latest blockbusters, ratings, trending reels, or box office hits!")
    ...
    if prompt := st.chat_input("Ask PopGuru (e.g., 'What are the reviews for the latest action movie?')...", key="popguru_chat"):
        ...
        with chat_container.chat_message("assistant"):
            with st.spinner("PopGuru is thinking..."):
                try:
                    ...
                    st.error(f"⚠️ API Error. Ensure your Gemini API Key is valid. Details: {e}")
