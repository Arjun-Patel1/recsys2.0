...
def process_data():
    ...
    ratings = pl.read_csv(
        config.RATINGS_PATH, columns=["userId", "movieId", "timestamp"]
    )
    ...
    active_users = user_counts.filter(pl.col("count") >= config.MIN_INTERACTIONS)[
        "userId"
    ]
    ...
    movie_map = pl.DataFrame(
        {
            ...
            "mapped_movieId": np.arange(
                1, len(unique_movie_ids) + 1
