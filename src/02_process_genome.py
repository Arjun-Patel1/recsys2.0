...
def process_genome():
    ...
    genome_pivot = genome.pivot(
        ...
        0
    )  # Fill missing tags with 0 relevance
    ...
    genome_joined = movie_map.join(
        genome_pivot, left_on="original_movieId", right_on="movieId", how="left"
    ).fill_null(
        0
    )  # Movies with no genome data get 0 vectors (Cold Start handling)

    # Extract just the tag columns (drop id columns)
    # Assuming tag columns start after 'mapped_movieId'
    tag_columns = [
        col
        for col in genome_joined.columns
        if col not in ["original_movieId", "mapped_movieId", "movieId"]
    ]
